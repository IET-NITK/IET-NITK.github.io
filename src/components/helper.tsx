...
import { Author } from "../types/templates";

export const lcrs = (link: string) => {
  return link.toLowerCase().split(" ").join("");
};


export const renderAuthors = (arr: Author[], cls: string) =>
  arr.map(({ name }, index2) => (
    <>
      &nbsp;
      <Link
...
export const renderAuthorsName = (arr: Author[]) =>
  arr.map(({ name }, index2) => (
    <>
      <Link
        className="pr-3"
...
export const renderProjectDescription = (element: string, len = 100) => {
...
export const newrenderAuthors = (arr: Author[]) =>
  arr.map(({ name }, index2) => (
    <>
      &nbsp;
      <Link
...
export const niceFormat = (name: string) =>
  name.toLowerCase().replace(/^\w|\s\w/g, (letter) => {
    return letter.toUpperCase();
  });

export function getRandom(arr: any[], num: number) {
