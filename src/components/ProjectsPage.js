...
const ProjectsPage = ({ projects, tag, img, sig }) => {
  ...
  const currentProjects = projects.filter(project => project.project_sig.toLowerCase() == sig.toLowerCase()).slice(indexOfFirstProject, indexOfLastProject);
  ...
  return (
    <main className='min-h-[90vh] relative'>
      ...
      <div className="flex gap-3 flex-col items-center mt-3">
        <div display="flex">
          <ul className="flex p-0 m-0 w-full bg-gray-200 rounded-lg list-none shadow-md">
